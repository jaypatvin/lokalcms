 ...
 *                   example: ok
 */
const received: RequestHandler = async (req, res) => {
  const data = req.body
  const { buyer_id } = data
  ...
  const notificationData = {
    type: 'order_status',
    title: 'Your products have been received',
    message: `The order (${order.products.length} products) has been received by the buyer.`,
