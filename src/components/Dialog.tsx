...
const Dialog = (props: DialogProps) => {
  const { isOpen, onClose, title, size, color = 'secondary', children } = props
  ...
  return (
    <>
      ...
      >
        <PosedDialogBoard
          pose={isOpen ? 'open' : 'closed'}
          className={`relative bg-white px-6 pt-3 pb-5 z-40 rounded shadow mx-auto mt-64 ${
            size ? dialogBoardSizes[size] : dialogBoardSizes['medium']
          }`}
          onClick={(e) => e.stopPropagation()}
        >
          <div>
            <button
              className="absolute -mt-4 mr-3 right-0 text-gray-600 hover:text-gray-800"
              onClick={onClose}
            >
              <IoIosClose size={30} />
            </button>
...
const ConfirmationDialog = (props: ConfirmationDialogProps) => {
  const { descriptions, onAccept, acceptLabel = 'Delete', cancelLabel = 'Cancel', ...rest } = props
  ...
  return (
    <Dialog {...rest}>
      ...
      <div className="flex justify-end">
        <div className="mr-2">
          <Button color={rest.color} size="small" onClick={onAccept}>
            {acceptLabel}
          </Button>
        </div>
        <div>
          <OutlineButton color={rest.color} size="small" onClick={rest.onClose}>
            {cancelLabel}
          </OutlineButton>
        </div>
      </div>
