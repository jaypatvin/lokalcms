...
const ProductListItem = ({
  ...
  if (data.availability) {
    ...
    if (repeat_unit === 0) {
      ...
      if (repeat_unit === 1) {
        if (repeat_type === 'day') availability = 'Every day'
        if (repeat_type === 'week') availability = `Every week on ${daysAvailable}`
        if (repeat_type === 'month')
          availability = `Every ${dayjs(start_dates[0]).format('Do')} of the month`
        if (nthDayOfMonthFormat.test(repeat_type)) {
          const [nth] = repeat_type.split('-')
          availability = `Every ${dayjs(`2021-01-${nth}`).format('Do')} ${dayjs(
            start_dates[0]
          ).format('dddd')}`
        }
      } else if (repeat_unit > 1) {
        if (repeat_type === 'day') availability = `Every ${repeat_unit} days`
        if (repeat_type === 'week') availability = `Every ${repeat_unit} weeks on ${daysAvailable}`
        if (repeat_type === 'month')
          availability = `Every ${dayjs(start_dates[0]).format(
            'Do'
  ...
  const getTileClass = ({ date }: CalendarTileProperties) => {
    const { start_dates, repeat_unit, repeat_type, schedule } = data.availability
